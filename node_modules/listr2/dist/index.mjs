...
let ListrTaskEventType = /* @__PURE__ */ function(ListrTaskEventType) {
	...
	*/
	...
	ListrTaskEventType["OUTPUT_RESET"] = "OUTPUT_RESET";
	/**
...
let ListrTaskState = /* @__PURE__ */ function(ListrTaskState) {
	...
	ListrTaskState["CANCELLED"] = "CANCELLED";
...
var EventManager = class {
	...
	off(dispatch, handler) {
		if (handler) this.emitter.off(dispatch, handler);
		else this.emitter.removeAllListeners(dispatch);
	}
	complete() {
...
let ListrLogLevels = /* @__PURE__ */ function(ListrLogLevels) {
	...
	ListrLogLevels["CANCELLED"] = "CANCELLED";
...
let ListrDefaultRendererLogLevels = /* @__PURE__ */ function(ListrDefaultRendererLogLevels) {
	...
	ListrDefaultRendererLogLevels["CANCELLED"] = "CANCELLED";
...
var DefaultRenderer = class DefaultRenderer {
	...
	async render() {
		...
		this.events.on("SHOUD_REFRESH_RENDER", this.refresh);
	}
	update() {
		...
		this.spinner.stop();
		this.events.off("SHOUD_REFRESH_RENDER", this.refresh);
		this.updater.clear();
		this.updater.done();
	...
	style(task, output = false) {
		...
		else if (task.isCancelled()) return this.logger.icon("CANCELLED");
	...
	setupBuffer(task) {
		...
		if (this.shouldOutputToBottomBar(task) && !this.buffer.bottom.has(task.id)) {
			...
			task.on("OUTPUT_RESET", () => {
				this.buffer.bottom.get(task.id)?.reset();
			});
			task.on("STATE", (state) => {
				if (state === "RETRY" || state === "ROLLING_BACK") this.buffer.bottom.delete(task.id);
			});
			...
			task.on("STATE", (state) => {
				if (state === "RETRY" || state === "ROLLING_BACK") this.buffer.output.delete(task.id);
			});
		}
	}
	reset(task) {
		...
		tasks.forEach((task) => {
			...
			task.on("STATE", (state) => {
				...
				if (state === "STARTED") this.logger.log("STARTED", task.title);
					...
					task.off("PROMPT");
					this.logger.process.release();
				} else if (state === "CANCELLED") this.logger.log("CANCELLED", task.title);
			});
			task.on("OUTPUT", (output) => {
...
var TaskWrapper = class {
	...
	*/
	report(error, type) {
		if (this.task.options.collectErrors) this.task.listr.errors?.push(new ListrError(error, type, this.task));
...
var Task = class extends ListrTaskEventManager {
	...
	*/
	set outputReset$(data) {
		this.output = data;
		this.emit("OUTPUT_RESET");
		this.listr.events.emit("SHOUD_REFRESH_RENDER");
	}
	/**
	...
	*/
	...
	async run(context, wrapper) {
		...
		try {
			...
			for (let retries = 1; retries <= retryCount; retries++) try {
				...
				if (this.listr.signal.aborted) throw err;
	...
	whenInterrupted() {
		return new Promise((_, reject) => {
			const onAbort = () => {
				if (!this.hasSubtasks()) reject(/* @__PURE__ */ new Error("Interrupted."));
			};
			if (this.listr.signal.aborted) {
				onAbort();
				return;
...
var Listr = class {
	...
	constructor(task, options, parentTask) {
		...
		} else this.errors = this.options.collectErrors ? [] : null;
		...
		if (this.parentTask?.listr.abortController instanceof AbortController) this.abortController = this.parentTask.listr.abortController;
		else {
	...
	*/
	async run(context) {
		...
		if (this.signal.aborted) {
			...
			throw error ?? /* @__PURE__ */ new Error("Interrupted.");
		}
